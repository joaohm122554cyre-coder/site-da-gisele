      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <Reveal>
          ...
          <h2 className="font-display text-4xl sm:text-5xl md:text-6xl text-[#1a2140] mt-4">
            Próximos <span className="italic text-[#d1548f]">shows</span>
          </h2>
        </Reveal>

        {upcoming.length === 0 ? (
          <Reveal delay={0.15}>
            <p className="mt-10 text-sm md:text-base text-[#1a2140]/55 max-w-md">
              Novas datas em breve. Para contratação de shows e eventos,
              entre em contato com a assessoria.
            </p>
            <a
              href="#contato"
              className="group inline-flex items-center gap-3 mt-10 text-[11px] tracking-[0.3em] uppercase text-[#1a2140]/75 hover:text-[#1a2140] transition-colors"
            >
          ...
          <ul className="mt-14 divide-y divide-[#a9a0d8] max-w-2xl">
            {upcoming.map((ev, i) => (
              <Reveal key={i} delay={i * 0.05}>
                <li className="flex justify-between py-5 text-[#1a2140]/75 text-sm md:text-base">
                  <span>{ev.date}</span>
                  <span>{ev.city}</span>
                </li>
              </Reveal>
